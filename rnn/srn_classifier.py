# Using Simple RNN to classify different poetry styles
...
class SimpleRNN(object):
	def __init__(self, M, V, activation=T.nnet.relu):
		...
		for i in range(epochs):
			...
			if debug:
				...
				for j in range(Nvalid):
					...
					n_correct_valid += (p == Yvalid[j])
				costs.append(cost)
				score = float(n_correct) / N
				score_valid = float(n_correct_valid) / Nvalid
	...
	def load(filename, activation=T.nnet.relu):
		...
		bo = npz['arr_5']
		V, M = Wx.shape
		rnn = SimpleRNN(M, V, activation)
		rnn.set(Wx, Wh, bh, h0, Wo, bo)
		return rnn


def train_poetry():
	X, Y, V = get_poetry_classifier_data(load_cached=True)
	print('\ntype(X):', type(X), 'type(Y):', type(Y))
	print('\nV = %d, len(X) = %d\n' % (V, len(X)))
	X, Y = shuffle(X, Y)
	Nvalid = 100
	Xvalid, Yvalid = X[-Nvalid:], Y[-Nvalid:]
	X, Y = X[:-Nvalid], Y[:-Nvalid]
	rnn = SimpleRNN(30, V, activation=T.nnet.relu)
	rnn.fit(X, Y, epochs=500, learning_rate=1e-4, decay=0.99999, debug=True, Xvalid=Xvalid, Yvalid=Yvalid)
