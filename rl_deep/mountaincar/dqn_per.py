# Use normal DQN with PER to solve MountainCar
...
class Brain:
    ...
    def __init__(self, input_sz, output_sz, hidden_layer_sizes, gamma, learning_rate=LEARNING_RATE, beta=BETA):
        ...
        if beta is None:
            ...
            cost = tf.reduce_sum(tf.square(self.targets - selected_action_values))
        else:
    ...
    def get_td_error(self, state, action, reward, next_state, done):
        next_output = np.max(self.predict(next_state))
    ...
    def optimize(self, batch_states, batch_actions, batch_rewards, batch_next_states, batch_dones, priorities):
        next_outputs = np.max(self.predict(batch_next_states), axis=1)
...
class Agent:
    ...
    def __init__(self, brain, num_actions, batch_sz, memory_capacity, alpha, eps_start, eps_end, eps_steps):
        self.brain = brain
    ...
    def train(self, state, action, reward, next_state, done):
        ...
        if self.memory.current_length() > self.batch_sz:
            ...
            td_errors = self.brain.optimize(batch_states, batch_actions, batch_rewards, batch_next_states, batch_dones, priorities)
...
if __name__ == '__main__':
    ...
    agent = Agent(brain, num_actions, BATCH_SZ, MEMORY_CAPACITY, ALPHA, EPS_START, EPS_STOP, EPS_STEPS)
