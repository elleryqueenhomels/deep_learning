# Gated Recurrent Unit (GRU) with Batch Training
...
class GRU(object):
	...
	def get_ht(self, xWxr_t, xWxz_t, xWxh_t, h_t1):
		r_t = T.nnet.sigmoid(xWxr_t + h_t1.dot(self.Whr) + self.br)
		z_t = T.nnet.sigmoid(xWxz_t + h_t1.dot(self.Whz) + self.bz)
		hhat = self.f(xWxh_t + (r_t * h_t1).dot(self.Whh) + self.bh)
		h_t = (1 - z_t) * h_t1 + z_t * hhat
		return h_t
