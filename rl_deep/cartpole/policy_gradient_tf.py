...
class HiddenLayer:
	def __init__(self, Mi, Mo, activation=tf.nn.tanh, use_bias=True):
...
# approximate pi(a | s)
class PolicyModel:
	def __init__(self, D, K, hidden_layer_sizes, activation=tf.nn.tanh):
		# create the graph
		# K == number of actions
		self.layers = []
		Mi = D
		...
		layer = HiddenLayer(Mi, K, activation=tf.nn.softmax, use_bias=False)
		self.layers.append(layer)
...
# approximate V(S)
class ValueModel:
	def __init__(self, D, hidden_layer_sizes, activation=tf.nn.tanh):
		# create the graph
		self.layers = []
		Mi = D
		for Mo in hidden_layer_sizes:
		...
		for layer in self.layers:
			Z = layer.forward(Z)
		Y_hat = tf.reshape(Z, [-1]) # reshape (N, 1) to (N, )
