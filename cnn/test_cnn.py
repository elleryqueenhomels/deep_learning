...
def main():
	...
	# Ytest_ind = y2indicator(Ytest, 10)
	Ntrain = int(len(Ytrain) / 4)
	Xtest, Ytest = Xtrain[Ntrain:], Ytrain[Ntrain:]
	Xtrain, Ytrain = Xtrain[:Ntrain], Ytrain[:Ntrain]
	...
	t0 = datetime.now()
	model.fit(Xtrain, Ytrain, epochs=5, batch_sz=500, learning_rate=10e-6, decay=0.99, momentum=0.9, reg_l2=0.01, debug=True, cal_train=True, debug_points=100, valid_set=[Xtest[:1000], Ytest[:1000]])
