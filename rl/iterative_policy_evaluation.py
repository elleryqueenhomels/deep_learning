...
def main():
	...
	# p(s',r|s,a) - the next state and reward given the action-state pair
	# we are only modeling p(a|s) = uniform
	# how would the code change if p(s',r|s,a) is not determiistic?
	grid = standard_grid()
	...
	while True:
		...
		for s in states:
			...
			# V(s) only have value if it's not a terminal state
