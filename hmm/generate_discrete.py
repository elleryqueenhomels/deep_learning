...
def generate_sequence(N):
	s = np.random.choice(M, p=pi) # initial state
	x = np.random.choice(V, p=B[s]) # initial observation
	sequence = [x]
	for n in range(N):
		s = np.random.choice(M, p=A[s]) # next state
		x = np.random.choice(V, p=B[s]) # next observation
		sequence.append(x)
	return sequence
